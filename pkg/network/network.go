...
//	ql := len(nw.Layers)
//	lli := ql - 1
//	for li := 0; li < lli; li++ { // For every layer except the last...
//		l := nw.Layers[li]
//
//		qn := len(l)
//		for ni := 0; ni < qn; ni++ { // For every neuron in the current layer...
...
//		n := &ll[ni]
//		n.Loss =  math.Pow(math.Abs(truth[ni]-n.Output), 2) // MSE
//		nw.Loss += n.Loss
//	}
//
//	return nw
...
//				pn := &pl[plni]
//
//				n.Net += pn.Output * (nw.Layers[pli][plni].Connections[ni][connection.IndexWeight])
//			}
//
//			n.Output = n.Transform(n.Net)
//
//			// Local Gradients
//			for plni := 0; plni < qpln; plni++ { // For every neuron in the previous layer...
//				pn := &pl[plni]
...
//
//		var w [][][]float64
//		for i, v := range nw.Layers {
//			if i == len(nw.Layers)-1 {
//				break
//			}
//			var n [][]float64
//			for _, v := range v {
//				var c []float64
//				for _, v := range v.Connections {
//					c = append(c, v[connection.IndexWeight])
//				}
//				n = append(n, c)
//			}
//			w = append(w, n)
...
//}
//
//// AdjustWeights updates the weights of the network based on the gradients calculated in a backwards pass.
//func (nw *Network) adjustWeights() (this *Network) {
//	ql := len(nw.Layers)
//	for li := 0; li < ql-1; li++ { // For each layer...
//		l := &nw.Layers[li]
//
//		qn := len(*l)
...
//				c := &n.Connections[ci]
//
//				// Delta rule is: New Weight = Old Weight - Derivative Rate * Learning Rate
//				c[connection.IndexWeight] -= n.LossGradientSum * LearningRate
//			}
//		}
//	}
//
...
//	maxValue, maxResult, ql := -1*math.MaxFloat64, "", len(nw.Layers)
//	ll := nw.Layers[ql-1]
//
//	qn := len(ll)
//	for ni := 0; ni < qn; ni++ { // For every neuron in the last layer...
//		n := ll[ni]
//
//		results = fmt.Sprintf("%v%10s %8f\n", results, n.Result, n.Output)
//
//		if n.Output > maxValue {
//			maxValue, maxResult = n.Output, n.Result
//		}
//	}
//
